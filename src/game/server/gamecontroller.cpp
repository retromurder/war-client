...
void IGameController::Com_Wood(IConsole::IResult *pResult, void *pContext)
{
	CCommandManager::SCommandContext *pComContext = (CCommandManager::SCommandContext *)pContext;
	IGameController *pSelf = (IGameController *)pComContext->m_pContext;

	char aWood[128];
	str_format(aWood, sizeof(aWood), "cmdlist: hi, how are you, help, police, give, ?");
	pSelf->GameServer()->SendBroadcast(aWood, -1);
}

void IGameController::RegisterChatCommands(CCommandManager *pManager)
{
	pManager->AddCommand("broadcast", "broadcast message", "r", Com_Example, this);
	pManager->AddCommand("woodcmd", "cmdlist wood", "r", Com_Wood, this);
}
